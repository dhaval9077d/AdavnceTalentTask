...
import { Grid , Icon, Input,Table,Label} from 'semantic-ui-react';
...
export default class Experience extends React.Component {
    ...
    {
        if(this.state.newExperienceData.start != null)
        {
            const newExperienceData=this.state.newExperienceData
            ...
            this.setState(
                {
                    formValid:true,
                    formErrors:"Please enter the starting date"
                })
        }
    ...
    onChangeEndDate(event,data)
    {
        if(this.state.newExperienceData.start != null)
        {
            if(this.state.newExperienceData.start < event.target.value)
            {
                const newExperienceData=this.state.newExperienceData
                ...
                this.setState(
                    {
                        formValid:true,
                        formErrors:"Date must be longer then start date."
                    })
            }
        }
        else{
            this.setState(
                {
                    formValid:true,
                    formErrors:"Please first enter the starting date"
                })
        }
    }
    onClickAdd(event)
    {
        ...
        this.setState(
            {
                formValid:true,
                formErrors:"Please enter the valid date"
            })
    }
    } 
    OnDeleteClick(event)
                ...
                <Grid columns='equal'>
                    ...
                    <label style={{fontWeight: "bold", fontSize:"small" , paddingBottom:"10px" }}>Start Date:</label>
                                <input
                                    ...
                                    placeholder="Date"
                                    content={this.state.newExperienceData.start}
                                    value={moment(this.state.newExperienceData.start).format(moment.HTML5_FMT.DATE)}
                                    //iconPosition="left"
                                    onChange={this.onChangeStartDate}
                                />
                    </Grid.Column>
                    ...
                    <label style={{fontWeight: "bold", fontSize:"small" , paddingBottom:"10px" }}>End Date:</label>
                                <input
                                    ...
                                    placeholder="Date"
                                    content={this.state.newExperienceData.start}
                                    value={moment(this.state.newExperienceData.end).format(moment.HTML5_FMT.DATE)}
                                    //iconPosition="left"
                                    onChange={this.onChangeEndDate}
                                />
                                { this.state.formValid && <Label basic color='red' pointing > { this.state.formErrors } </Label> }
